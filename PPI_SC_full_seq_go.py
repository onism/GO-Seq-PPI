...
def main():
    ...
    train_pairs_file = 'SC_CV/all_train'+str(rep)+'-'+str(split)
    test_pairs_file = 'SC_CV/all_test'+str(rep)+'-'+str(split)
    valid_pairs_file = 'SC_CV/all_valid'+str(rep)+'-'+str(split)
    ...
    model = build_model()
    save_model_name = 'SC_CV/all_sc_GoplusSeq'+str(rep)+'-'+str(split) + '.hdf5'
...
#     if int(rep) >= 0 and int(split) > 2:
    ...
    y_true = np.zeros((test_len,1))
    y_pred_prob = np.zeros((test_len,1))
    for index in range(int(np.floor(test_len/batch_size))):
        ...
        y_true[start_ind:end_ind] = y_test.reshape(len(indexes),1)
    ...
    np.savez('SC_CV/SC_seq_'+rep+'-'+split, AUCs=auc, ACCs=acc, MCCs=mcc, TPRs = tpr, FPRs=fpr, Precs=pre, F1s=f1)
